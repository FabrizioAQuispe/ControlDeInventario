...
import { getConnection, sql } from "../constant/db";
...
export async function LISTAR_DNIS(dni: string) {
  try {
    ...
    console.log("=============LISTANDO LOS TIPOS DE ANÁLICIS ===============")
    return result.recordsets
  } catch (e) {
    console.error('Error en LISTAR_DNIS:', e);
    throw e;
  }
}
export async function LISTAR_CUENTA_CORRIENTE(ccd01cod: string) {
  try {
    const pool = await getConnection();
    const result = await pool.request()
      .input("dni", ccd01cod) // Enlazas parámetro
      .query(`
SELECT * FROM ccm02cta WHERE ccm02cod = @dni`);
